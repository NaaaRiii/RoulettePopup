...
//      const effectiveTaskId = task.id || `temp-${index}`;
...
//  <div key={task.id ? task.id : `temp-${index}`}>
//    <label htmlFor={`task-${task.id ? task.id : `temp-${index}`}`}>Task</label>
//    <input
//      id={`task-${task.id ? task.id : `temp-${index}`}`}
//      type="text"
//      name="content"
//      value={task.content}
//      onChange={(e) => handleChange(e, task.id || `temp-${index}`)}
//      required
//    />
//    <button type="button" onClick={() => removeTask(task.id)}>Remove Task</button>
//  </div>
