describe('本番環境でのルーレット回転テスト', () => {
  ...
  it('本番環境でルーレットを回すことができる', () => {
    // ページが正常に読み込まれているか確認
    cy.get('body').should('be.visible')
    
    // ルーレットコンテナが表示されているか確認
    cy.get('[data-testid="roulette-container"]', { timeout: 10000 }).should('be.visible')
    
    // ルーレットボタンが存在するか確認
    cy.get('[data-testid="start-button"]', { timeout: 10000 })
      .should('be.visible')
